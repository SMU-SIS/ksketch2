...
package sg.edu.smu.ksketch2.canvas.components.timebar
{
	...
	{	
		...
		private function _drawTicks():void
		{
			...
			{
				_timeControl.unselectedTickMarkDisplay.graphics.lineStyle(KSketchStyleSheet.TIME_TICK_THICKNESS, KSketchStyleSheet.ACTIVITY_OTHER_COLOR);
					...
					if(currentX <= currentMarker.x)
					{
						...
						if(firstFrame)
						{
							if(firstFrame.passthrough)
								_timeControl.selectedTickMarkDisplay.graphics.lineStyle(KSketchStyleSheet.TIME_TICK_THICKNESS_A, KSketchStyleSheet.TIME_TICK_CONTROLPOINT);
							else
								_timeControl.selectedTickMarkDisplay.graphics.lineStyle(KSketchStyleSheet.TIME_TICK_THICKNESS_B, KSketchStyleSheet.TIME_TICK_KEYFRAME);
						}
						else
							_timeControl.selectedTickMarkDisplay.graphics.lineStyle(KSketchStyleSheet.TIME_TICK_THICKNESS_A, KSketchStyleSheet.TIME_TICK_CONTROLPOINT);
						...
						if(drawTarget.x <= currentX)
						{
							...
							if(currentMarker.prev)
							{
								if(currentMarker.key is ISpatialKeyFrame)
								{
									...
									{
										drawTarget.graphics.beginFill(KSketchStyleSheet.ACTIVITY_COLOR, 0.6);
			...
			{
				_timeControl.unselectedTickMarkDisplay.alpha = KSketchStyleSheet.TIME_TICK_SELECTED_ALPHA;
				_timeControl.unselectedTickMarkDisplay.graphics.lineStyle(KSketchStyleSheet.TIME_TICK_THICKNESS_A, KSketchStyleSheet.ACTIVITY_OTHER_COLOR);
			}
